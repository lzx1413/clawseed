...
class GatewayService : Service() {
    ...
    override fun onCreate() {
        ...
        startForeground(NOTIFICATION_ID, buildNotification("Starting ClawSeed gateway..."))
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (serviceJob == null) {
            serviceJob = scope.launch {
                gateway.start()
                if (gateway.state.value is GatewayState.Running) {
                    isReady = true
                    updateNotification("Gateway running on :${(gateway.state.value as GatewayState.Running).port}")
                    withContext(Dispatchers.Main) {
                        readyCallbacks.forEach { it() }
                        readyCallbacks.clear()
                    }
                } else {
                    updateNotification("Gateway failed to start")
                }
            }
        }
        return START_STICKY
