...
**相关性与排序**：`MemoryEntry.score` 和 `min_relevance_score` 使用 0–1 范围的查询相关性，与 BM25、RRF、加权融合的排序分数分离。关键词相关性按 key/content 覆盖的有效查询词比例计算，查询词去重，单字及常见英文虚词不作为相关性依据；拉丁词要求完整词匹配，中文/日文多字短语允许子串匹配。混合搜索取关键词覆盖率与向量余弦相似度中的较大值。这些是启发式相关性指标，并非概率。先过滤相关性，再截取最终结果数。

**兜底机制**：Hybrid/Bm25 返回空结果时，可对完整查询做字面量 LIKE 搜索（content/key 列），转义 `%`、`_` 和反斜杠。兜底结果仍经过同一关键词相关性过滤，不再统一赋予满分。Embedding 模式不会回退到关键词搜索。带相关性阈值的空查询返回空结果；显式、不带阈值的浏览仍可列出最近记忆。

**自动上下文**：“你好”“您好”“Hello”“好的”“嗯嗯”“OK”等独立问候或确认语句跳过动态召回。候选扩大到最终条数的 4 倍后再次过滤：无关键词支持时，语义相关性至少达到 max(配置阈值, 0.65) 才自动注入；显式 memory_recall 保持较宽的检索。0.65 是保守过滤门槛，不是相关概率保证。没有合格结果时，不新增 `[Memory context]` 区块。历史区块保留在原消息中，维持提示前缀稳定。Android Debug 分别标注系统提示、历史消息和本轮消息，保留原始内容和顺序。
...
**常驻记忆索引**：启用 `stable_memory_in_system_prompt` 时，系统只放排序后的 Core 查询键及历史记录说明，不再常驻完整事件正文。索引中的条目仍可在相关问题中召回。用户画像与记忆索引位于固定工具/安全/技能目录说明之后，减少变化对公共前缀的影响。历史对话中的正文不改写。
