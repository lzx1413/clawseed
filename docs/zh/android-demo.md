...
- **分身系统**为新会话绑定专注于特定任务的助手。每个分身可以覆盖 Soul、模型、思考模式、
  记忆命名空间、可用工具，以及启用或禁用的技能。
- **用户画像**以结构化条目保存长期偏好和上下文。画像既可以人工维护，也可以在成功回复后
  自动推断；用户可以审核、拒绝不正确的推断，并随备份导入导出。
- **语音播报**使用 Android Text-to-Speech 朗读助手回复。用户既可以全局启用自动朗读，
  也可以在会话中播放或停止单条回复。
- **定时任务**可以在后台执行提示，并保留自己的会话，因此执行期间仍可使用设备工具和
  CETP 外部工具。
- **跨应用工具**通过 CETP 自动发现并桥接到当前工具注册表。[九财花联动](integrations/jiucaihua.md)
  展示了使用真实投资数据完成工具调用闭环的方式。
...
设置中的“助手记住的内容”进入统一页面，包含“关于我”和“记忆”两个标签页。“关于我”支持类别、推断来源和已拒绝状态筛选，以及多选删除/拒绝、删除全部推断项、限时变更预览、冲突刷新和应用后的单次撤销。Activity 重建后会保留待确认计划的标识，但绝不会自动应用计划。记忆条目显示类别与 namespace，使 Persona 作用域保持可见。

聊天中的画像工具结果使用相同的结构化预览和动作控件，不解析模型文本。破坏性操作必须确认。

画像和记忆均属于个人数据。启用隐私过滤的导出会同时排除 `USER_PROFILE` 与 `MEMORY`；完整归档可能以明文包含两者，必须按敏感数据存储和分享。应用覆盖升级不要求清除应用数据或从归档恢复。

在 Android 16 上，应用会先请求 `NEARBY_WIFI_DEVICES`，再启动嵌入式回环 Gateway。Android 16 启用本地网络保护试验开关时会使用该权限；缺少权限会导致本地监听以 `EPERM` 失败。

## 图片对话

配置 DeepSeek，模型选择 `deepseek-v4-flash-vision-exp`。连接会话后点击“添加图片”，从相册选取最多 4 张，检查缩略图、移除或重试上传；支持纯图片及图文发送。点击已发送图片放大并双指缩放。旧 Gateway 会明确提示不支持图片上传。

图片草稿副本保存在 App 文件目录，按 Gateway 地址和会话隔离。处理时校正方向，按需缩小并转成 PNG，尽量保留截图文字清晰度；GIF 使用静态帧。发送失败保留图片草稿，进程重启后也可重试。历史图片通过会话对应的 Gateway 认证读取，App 或 Gateway 重启后仍可恢复。旧图超出模型图片上下文时会提示重新附图，但历史仍可查看。

### 视觉能力配置

LLM 设置提供“自动 / 开启 / 关闭”。旧配置默认自动，继续识别原有 DeepSeek
视觉模型；其他未知模型需手动指定。本阶段尚未接入服务商元数据发现或测试图探测。

分身可以继承或独立设置视觉能力。继承仅用于同一模型，更换模型后按自动判断。
附件图标根据当前会话实际 Provider 的能力启用；未知或不支持时置灰且不能点击。
Gateway 在 `session_start`（包括恢复会话）返回这一判断，并在处理图片时再次校验。
分身修改在新建连接后生效。
