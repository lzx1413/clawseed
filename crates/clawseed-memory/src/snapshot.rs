...
//!
//! **Soul Export**: dumps `MemoryCategory::Core` from SQLite into
//! `MEMORY_SNAPSHOT.md` so the agent's "soul" is always recoverable.
//!
...
/// Filename for the snapshot (lives at workspace root).
pub const SNAPSHOT_FILENAME: &str = "MEMORY_SNAPSHOT.md";

/// Header written at the top of every snapshot file.
const SNAPSHOT_HEADER: &str = "# ClawSeed Memory Snapshot\n\n\
    > Auto-generated by ClawSeed. Do not edit manually unless you know what you are doing.\n\
...
/// Export all `Core` memories from SQLite to `MEMORY_SNAPSHOT.md`.
///
...
pub fn export_snapshot(workspace_dir: &Path) -> Result<usize> {
    ...
    tracing::info!(
        "Memory snapshot exported: {} core memories to {}",
        rows.len(),
...
mod tests {
    ...
    fn parse_snapshot_basic() {
        let input = r#"# ClawSeed Memory Snapshot

> Auto-generated by ClawSeed.
...
I am ClawSeed, a self-preserving AI agent.
...
"#;
    ...
    fn parse_snapshot_empty() {
        let input = "# ClawSeed Memory Snapshot\n\n> Nothing here.\n";
        let entries = parse_snapshot(input);
...
"#;
    ...
    fn export_and_hydrate_roundtrip() {
        ...
        // Create brain.db with some core memories
        ...
        // Simulate data loss: delete brain.db
        fs::remove_file(&db_path).unwrap();
        assert!(!db_path.exists());

        // Verify should_hydrate
