...
impl AnthropicProvider {
    ...
    pub fn with_base_url(credential: Option<&str>, base_url: Option<&str>) -> Self {
        ...
        Self {
            ...
            base_url,
            max_tokens: clawseed_api::provider::BASELINE_MAX_TOKENS,
        }
    }
    ...
    fn convert_messages(messages: &[ChatMessage]) -> (Option<SystemPrompt>, Vec<NativeMessage>) {
        ...
        let mut system_stable_prefix: Option<String> = None;
            ...
            match msg.role.as_str() {
                "system" => {
                    if system_text.is_none() {
                        ...
                        system_stable_prefix = msg.stable_prefix.clone();
                    }
                }
                "assistant" => {
        ...
        // Build system prompt from partitioned or legacy format
        let system_prompt = system_text.map(|text| {
            if let Some(stable) = system_stable_prefix
                && !stable.is_empty()
                && text.starts_with(&stable)
                && stable.len() < text.len()
            {
                // Partitioned: split at exactly stable.len(). The preamble (if any)
                // is already part of the stable prefix (see prompt.rs
                // build_partitioned), so we only trim the "\n\n" separator
                // between the two halves.
                let dynamic = text[stable.len()..].trim_start_matches('\n').to_string();
                SystemPrompt::Blocks(vec![
                    ...
                    SystemBlock {
                        block_type: "text".to_string(),
                        text: dynamic,
                        cache_control: None, // Dynamic block NOT cached
                    },
                ])
            } else {
                // Legacy: single block with cache_control
...
impl Provider for AnthropicProvider {
    ...
    async fn chat(
        ...
        let tool_choice_override = clawseed_api::TOOL_CHOICE_OVERRIDE
...
mod tests {
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are ClawSeed".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are ClawSeed\""));
    }
    ...
    // NOTE: Disabled because `axum` is not a dev-dependency of clawseed-providers.
    // Re-enable after adding axum to [dev-dependencies] in Cargo.toml.
    #[cfg(any())]
    #[tokio::test]
    async fn _disabled_chat_with_tools_sends_full_history_and_native_tools() {
