            ...
            && let Some(message) = output.message
        {
            for block in message.content {
                match block {
                    ...
                    ResponseContentBlock::Other(v) => drop(v),
                }
            }
        }
...
mod tests {
    use super::*;
    // NOTE: crate::test_util does not exist in clawseed; tests requiring
    // EnvGuard/env_lock are disabled below.
    ...
    // NOTE: Disabled - requires crate::test_util::env_lock which doesn't exist.
    #[cfg(any())]
    #[tokio::test]
    async fn _disabled_chat_fails_without_credentials() {
        // let provider = {
    ...
    // NOTE: Disabled - requires crate::test_util::{EnvGuard, env_lock}.
    #[cfg(any())]
    #[test]
    fn _disabled_bearer_token_from_env() {
        // let _env_lock = env_lock();
    ...
    #[cfg(any())]
    #[test]
    fn _disabled_bearer_token_precedence() {
        // let _env_lock = env_lock();
