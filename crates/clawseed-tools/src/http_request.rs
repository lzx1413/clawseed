...
impl HttpRequestTool {
    ...
    async fn execute_request(
        ...
        let builder =
            clawseed_config::schema::apply_runtime_proxy_to_builder(builder, "tool.http_request");
...
mod tests {
    ...
    #[tokio::test]
    async fn execute_blocks_when_no_domains() {
        let tool = HttpRequestTool::new(vec![], 1_000_000, 30, false);
        let result = tool
            .execute(json!({"url": "https://example.com"}), &test_ctx())
            .await
            .unwrap();
        assert!(!result.success);
        assert!(result.error.unwrap().contains("allowed_domains"));
    }
