...
impl GitOperationsTool {
    ...
    fn _requires_write_access(&self, operation: &str) -> bool {
        matches!(
            operation,
...
impl Tool for GitOperationsTool {
    ...
    fn description(&self) -> &str {
        "Perform structured Git operations (status, diff, log, branch, commit, add, checkout, stash). Provides parsed JSON output for structured repository management."
    }
...
mod tests {
    ...
    fn requires_write_detection() {
        ...
        assert!(tool._requires_write_access("commit"));
        assert!(tool._requires_write_access("add"));
        assert!(tool._requires_write_access("checkout"));

        assert!(!tool._requires_write_access("status"));
        assert!(!tool._requires_write_access("diff"));
        assert!(!tool._requires_write_access("log"));
    }
    ...
    fn branch_is_not_write_gated() {
        ...
        assert!(!tool._requires_write_access("branch"));
    ...
    async fn blocks_readonly_mode_for_write_ops() {
        ...
        // This test just verifies the tool doesn't crash
    }
