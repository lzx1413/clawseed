...
/// Agent should handle unknown tool name without crashing
#[tokio::test]
...
/// Agent should not exceed max_tool_iterations even with
...
async fn agent_respects_max_tool_iterations() {
    ...
    let provider = Box::new(common::mock_provider::MockProvider::new(responses));
