...
//! JSON to ClawSeed's system prompt format.
...
mod tests {
    ...
    #[test]
    fn parse_aieos_identity_supports_generator_shape() {
        let json = r#"{
            "identity": {
            ...
            "psychology": {
                "neural_matrix": { "creativity": 0.55, "logic": 0.62 },
                "traits": { "ocean": { "openness": 0.4 }, "mbti": "ISFJ" },
                "moral_compass": {
        ...
        let core = identity.identity.unwrap();
        assert_eq!(core.names.unwrap().first.as_deref(), Some("Marta"));
        assert!(core.bio.unwrap().contains("Female"));

        let psych = identity.psychology.unwrap();
        assert_eq!(psych.mbti.as_deref(), Some("ISFJ"));
        assert_eq!(psych.ocean.unwrap().openness, Some(0.4));
        assert!(
            psych
                .moral_compass
                .unwrap()
    ...
    #[test]
    fn load_aieos_identity_from_file() {
        let json = r#"{"identity":{"names":{"first":"Nova"}}}"#;
    ...
    #[test]
    fn aieos_to_system_prompt_sorts_hashmaps() {
        let mut neural_matrix = HashMap::new();
