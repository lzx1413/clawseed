//! Dispatcher: parses tool calls from LLM responses and formats results back.
//!
...
/// XML-based tool dispatcher for prompt-guided tool calling.
#[derive(Default)]
pub struct XmlToolDispatcher;

impl XmlToolDispatcher {
    const TOOL_CALL_START: &'static str = "◁";
    ...
    fn parse_xml_tool_calls(response: &str) -> (String, Vec<ParsedToolCall>) {
        ...
        while let Some(start) = remaining.find(Self::TOOL_CALL_START) {
            let before = &remaining[..start];
            if !before.trim().is_empty() {
                text_parts.push(before.trim().to_string());
            }

            let after_start = &remaining[start + Self::TOOL_CALL_START.len()..];
            if let Some(end) = after_start.find(Self::TOOL_CALL_END) {
                ...
                match serde_json::from_str::<Value>(inner.trim()) {
                    Ok(parsed) => {
                        let name = parsed
                            ...
                            .to_string();
                        if name.is_empty() {
                            remaining = &after_start[end + Self::TOOL_CALL_END.len()..];
                            continue;
                        }
...
/// Native tool dispatcher for providers that support tool calling natively.
