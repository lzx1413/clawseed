...
async fn run_agent_job(
                ...
                Ok(entries) if !entries.is_empty() => {
                    let ctx: String = entries
                        ...
                        .filter(|e| {
                            !matches!(
                                e.category,
                                clawseed_memory::traits::MemoryCategory::Conversation
                            )
                        })
    ...
    let _model_override = job.model.clone();
    ...
    let run_result: Result<String, String> = match job.session_target {
        SessionTarget::Main | SessionTarget::Isolated => {
            // Agent job execution requires a provider, which is not available
            // in the minimal agent crate. The binary crate should register
            // a runner via `register_agent_runner`.
            match run_agent_job_impl(&prefixed_prompt) {
                Some(fut) => fut.await,
                None => Err("agent jobs require a registered runner (register_agent_runner was not called by the binary)".to_string()),
            }
        }
    };
...
/// Agent runner function type — takes a prompt and returns the agent's response.
...
/// Global agent runner function, injected by the binary crate at startup.
static AGENT_RUNNER_FN: std::sync::OnceLock<AgentRunnerFn> = std::sync::OnceLock::new();

/// Register the agent runner function. Called once at startup by the binary.
pub fn register_agent_runner(f: AgentRunnerFn) {
    let _ = AGENT_RUNNER_FN.set(f);
}

/// Try to get the registered agent runner and run a prompt.
...
///
/// Uses `sh -c <command>` (non-login shell). On Windows, users
...
mod tests {
    ...
    async fn run_job_command_blocks_readonly_mode() {
        let tmp = TempDir::new().unwrap();
        let mut config = test_config(&tmp).await;
        config.autonomy.level = clawseed_config::schema::AutonomyLevel::ReadOnly;
    ...
    async fn process_due_jobs_marks_component_ok_even_when_idle() {
        // In the minimal agent crate, health monitoring is a no-op.
        // This test verifies that the health module doesn't panic when called.
        let tmp = TempDir::new().unwrap();
        let config = test_config(&tmp).await;
        let security = Arc::new(SecurityPolicy::from_config(
            &config.autonomy,
    ...
    // scan_and_redact_output tests moved to channels orchestrator
